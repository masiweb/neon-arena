...
async def main() -> None:
    ...
    async with (
        websockets.connect(f"{BASE_WS}/ws/{code}?name=One&protocol=3&client=android", proxy=None) as first,
        websockets.connect(f"{BASE_WS}/ws/{code}?name=Two&protocol=3&client=web", proxy=None) as second,
    ):
